...
enum TokenType:
  ...
  case LeftParen, RightParen, LeftBrace, RightBrace, LeftSquare, RightSquare,
       ...
       Minus, Plus, Star, Slash, Percent, Or, And, Equal, Xor, Bang, Tilde,
       Greater, Less,
  ...
  //Built-in Types
       Word, Boolean, Ptr,
  //Keywords
       Abs, Assembly, Break, Decrement, Else, False, From, Func, For, If,
       Include, Increment, Load, Null, Return, True, While,
...
object Token{
  ...
  val typeMap: Map[String, TokenType] = Map(
    "word" -> Word,
    "bool" -> Boolean,
    "ptr" -> Ptr
  )
